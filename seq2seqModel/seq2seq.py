...
def build_decoder2(lstm_outputs, final_utterance_embedding):
    """
    build the computational graph for the FF decoder, based on Guu et al. Return only the palceholders and tensors
    that are called from other methods. Names of marics and vectors follow those in the paper.
    """
    ...
    W_a2 = tf.get_variable("W_a", shape=[DECODER_HIDDEN_SIZE_NEW, SENT_EMB_SIZE],
                          initializer=tf.contrib.layers.xavier_initializer())
    alpha2 = tf.nn.softmax(tf.matmul(tf.matmul(tf.transpose(q_t2), W_a2), tf.transpose(lstm_outputs)))
    c_t2 = tf.matmul(alpha2, lstm_outputs)  # attention vector
    W_s2 = tf.get_variable("W_s", shape=[DECODER_HIDDEN_SIZE_NEW, DECODER_HIDDEN_SIZE_NEW + SENT_EMB_SIZE],
                          initializer=tf.contrib.layers.xavier_initializer())
    W_hidden2 = tf.get_variable("W_hidden", dtype=tf.float32,
...
def run_model(sess, dataset, mode, validation_dataset=None, load_params_path=None, save_model_path=None,
              ...
              beam_reranking_train=False, beam_similarity=False, beam_similarity_test=False, similarity_params_path=None):
    """ 
    ...
    if similarity_params_path:
        tf.train.Saver(var_list=theta2).restore(sess, similarity_params_path)
    # load pre-trained variables, if given
    if load_params_path:
        tf.train.Saver(var_list=theta1).restore(sess, load_params_path)
    if save_model_path:
        saver2 = tf.train.Saver(max_to_keep=10, keep_checkpoint_every_n_hours=1)

    # initialize gradient buffers to zero
    gradList = sess.run(theta1)  # just to get dimensions right
    gradBuffer = {}
    ...
    similarity_ran_already = False
    top_program_by_similarity = 0
    ...
    while dataset.epochs_completed < n_epochs and not stopping_criterion_met:
        ...
        for step in range(len(sentences)):
            ...
            if beam:
                ...
                # check in how many cases the highest prob program is wrong but there is a correct program in the beam 
                correct_in_beam = 0
                if programs_execution_results[beam[0]].is_consistent:
                    check_reranker_potential = False
                else:
                    check_reranker_potential = True

                if beam_similarity and curr_mode =='test':
                    similarity_scores = []
                    for prog in beam:
                        ...
                        if beam_similarity_test:
                            ...
                            #with tf.Session(graph=g_3) as sess3:
                                #similarity_scores.append(run_similarity_model(sess3,curr_example,
                                                                              ...
                                                                              #load_params_path=similarity_params_path,
                                                                              #inference=True, reuse=similarity_ran_already))
                            #similarity_ran_already = True

                            sentence_matrix = np.stack(
                                [one_hot_dict.get(w, one_hot_dict['<UNK>']) for w in sentence.split()])
                            bow_words = np.reshape(np.sum([words_array == x for x in sentence.split()], axis=0),
                                                   [1, len(words_vocabulary)])

                            length = [len(sentence.split())]

                            history_tokens = ['<s>' for _ in range(MAX_DECODING_LENGTH - len(prog.token_seq))] + \
                                             prog.token_seq[-MAX_DECODING_LENGTH:]

                            history_embs = [logical_tokens_embeddings_dict[tok] for tok in history_tokens]
                            history_embs = np.reshape(np.concatenate(history_embs),
                                                      [1, MAX_DECODING_LENGTH * LOG_TOKEN_EMB_SIZE])
                    ...
                    if beam_similarity_test:
                        ...
                        top_by_similarity_stats = programs_execution_results[top_program_by_similarity]
                ...
                if beam_classifier:
                    ...
                    for prog in beam:
                        ...
                        label = programs_execution_results[prog].is_consistent
                        if label and check_reranker_potential:
                            correct_in_beam = 1
                        features.append(feat_vector)
                        labels.append(label)
                    all_features.append(features)
                    all_labels.append(labels)
                    total_potential += correct_in_beam

                    if beam_classifier_test:
                        with tf.Session(graph=g_2) as sess2:

                            classifier_best_index = run_beam_classification(sess2, [features], [labels], BEAM_SIZE,
                                                                            inference=True,
                                                                            load_params_path=clf_params_path,
                                                                            reuse=classifier_ran_already)
            ...
            if mode == 'test' and not beam_reranking_train:
                stats_for_all_sentences[sentence_id] = {'top_program_by_reranking': top_program_by_reranking,
                                                        ...
                                                        'top_program_by_similarity': top_program_by_similarity,
                                                        'top_by_similarity_stats': top_by_similarity_stats,
            ...
            if beam_similarity and beam_similarity_test:
                n_correct_top_by_similarity.append(top_by_similarity_stats.n_correct)
                n_consistent_top_by_similarity.append(top_by_similarity_stats.is_consistent)
...
def run_supervised_training(sess, load_params_path=None, save_params_path=None, num_epochs=MAX_N_EPOCHS):
    ...
    while epoch_num < num_epochs:
        if current_data_set.epochs_completed != epoch_num:
            statistics[current_data_set].append(
        ...
        for step in range(batch_size):
            ...
            encoder_feed_dict, decoder_feed_dict = \
                get_feed_dicts_from_sentence(sentence, sentence_placeholder, sent_lengths_placeholder,
                                             sentence_words_bow, (h, e_m), LEARN_EMBEDDINGS_IN_PRETRAIN)
            ...
            accuracy_chosen_tokens.append(sum(correct_greedy_choices[i] for i in range(len(golden_parsing))
                                          ...
                                          len(golden_parsing) - len(no_real_choice_indices)))
        ...
        if beam_reranking_train:
            with tf.Session() as sess:
                ...
                features = pickle.load(
                    open(os.path.join(SEQ2SEQ_DIR, 'features_for_beam_reranking2017-11-10_20_31'), 'rb'))
