...
该模块包含执行逻辑表单所需的方法（也称为“程序”）
关于图像的结构化表示。在我们使用的形式主义中，逻辑形式是一系列令牌，
each表示某种类型的值或具有预定义arity和参数类型的函数。
因此，每个这样的有效逻辑形式（在向其添加括号和逗号之后）是可执行文件
python代码行。
请注意，这里没有定义我们的形式主义的实际语法和允许的令牌集。
但是，任何这样的令牌集必须是此处定义的函数的子集。
"""
